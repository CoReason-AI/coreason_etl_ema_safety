...
def discover_ema_excel_urls() -> list[str]:
    ...
    """
    # The FRD specifies we should "target the underlying hidden XHR/API JSON endpoint"
    # But currently the EMA serves them directly in the HTML of the download page.
    # To be safe and compliant, we will fetch the page and extract URLs, checking for the required filters.

    # We'll fetch the HTML/JSON response
    # The endpoint needs an Accept header for JSON to return the ajax drupal response
    headers = {"Accept": "application/json"}
    response = requests.get(EMA_DISCOVERY_URL, headers=headers, timeout=30)
    response.raise_for_status()

    # Regex to find all matching xlsx documents
    # Example: /en/documents/report/medicines-output-medicines-report_en.xlsx
    # The JSON response contains HTML strings encoded or escaped, re.finditer handles raw search nicely
...
def process_ema_excel(url: str) -> Iterator[dict[str, Any]]:
    ...
    try:
        ...
        for row in df.to_dicts():
            yield {"coreason_id": row.get("coreason_id"), "source_file_url": url, "raw_data": row}
